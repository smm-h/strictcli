...
- `CoRequired(flags=[...])` declares flags that must appear together. `Requires(flag=..., depends_on=...)` declares one-way dependency. `Implies(flag=..., implies=..., value=...)` auto-sets a bool flag when a trigger is provided. All passed via `dependencies=[...]`. They operate at root scope only -- a constraint naming a scoped flag is a registration error, because the scope already is the constraint.
...
- Failures (unknown command, missing required flags, election and scope violations, dependency errors) produce `InvokeError` -- returned as the error in Go, raised as an exception in Python. A choice flag's value is the same record a handler receives: a choice instance (Python), `Elect(<choice>, Fields{...})` (Go), the union member object (TypeScript).
