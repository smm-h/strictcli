...
- **typescript/src/elected** (`typescript/src/elected.ts`): The delivered elected record: the tagged value a selector hands a handler, plus the two helpers a handler uses to consume it (contract §24.1, §24.9).
...
- **typescript/src/infer** (`typescript/src/infer.ts`): Handler-args inference: computes the precise args object type a handler receives from a command's keyed declaration map and ordered arg tuple.
...
- **typescript/src/schema** (`typescript/src/schema.ts`): Schema dump (--dump-schema): builds the machine-readable schema dict and writes .strictcli/schema.json describing every command, group, flag, and arg, at SCHEMA VERSION 2 (contract §25).
- **typescript/src/scopeparse** (`typescript/src/scopeparse.ts`): The scoped-selector parser (contract §24.3): elections, scope-membership validation, and per-scope value resolution.
