...
- **typescript/src/constraints** (`typescript/src/constraints.ts`): The constraint system (contract §26): the two co-occurrence families resolved against their command, rendered for every surface that publishes them, and evaluated at parse time.
...
- **typescript/src/update** (`typescript/src/update.ts`): The update-command construct after registration (contract §27).
