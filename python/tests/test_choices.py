...
def test_valid_choice_accepted():
    ...
    app = _make_app_with_choices(choices=[strictcli.Choice("text"), strictcli.Choice("json")])
...
def test_choices_shown_in_help():
    ...
    app = _make_app_with_choices(choices=[strictcli.Choice("text"), strictcli.Choice("json")], default="text")
    r = app.test(["cmd", "--help"])
    assert r.exit_code == 0
...
def test_choices_with_int_type():
    ...
    @app.command("cmd", effect="read_only", help="a command")
    @strictcli.flag("port", type=int, help="the port", choices=[strictcli.Choice(80), strictcli.Choice(443), strictcli.Choice(8080)], presence="required")
    def cmd(ctx, port):
        print(f"port={port}")
...
def test_choices_with_env_var_valid(monkeypatch):
    ...
    @app.command("cmd", effect="read_only", help="a command")
    @strictcli.flag(
        "format", help="output format", choices=[strictcli.Choice("text"), strictcli.Choice("json")],
...
def test_default_must_be_in_choices():
    ...
    with pytest.raises(ValueError, match="default.*not in choices"):
        strictcli.Flag(
            ...
            choices=[strictcli.Choice("text"), strictcli.Choice("json")], default="xml",
        )
...
def test_choices_with_bool_raises():
    ...
    with pytest.raises(ValueError, match="incompatible with type=bool"):
        strictcli.Flag(
            ...
            choices=[strictcli.Choice(True), strictcli.Choice(False)], presence="required",
        )
